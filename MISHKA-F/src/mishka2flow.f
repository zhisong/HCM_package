...
C                                                                       
      ...
      PARAMETER (LVANZ=11, MVANZ=11, LVMAX=LMAX)                        
...
C                                                                       
C------------------------------------------ READ ADDITIONAL BOUNDARY DAT
      IF(RWALL.GT.1.) THEN                                              
...
C-----------------------------------------------------------------------
C   R**4*(GRAD.PSI*GRAD.THETA)**2 / GRAD.PSI**2 -->  RR4GPTOGP, IR4GPTOG
C-----------------------------------------------------------------------
      DO I=1,NGES                                                       
...
C-----------------------------------------------------------------------
C   R**2*GRAD.PSI*GRAD.THETA**2 /(1+GRAD.PSI**2/F**2) --> RFR2GT2,IFR2GT
C-----------------------------------------------------------------------
      DO 225 I=1,NGES                                                   
...
C-----------------------------------------------------------------------
C   R**2*GRAD.PSI**4*(G.PSI*G.THETA) *  DS R**2  --> RR2GP2GPTDSR2,IR2GP
C-----------------------------------------------------------------------
      DO J=1,NCHI                                                       
         CALL SGCOPY(NPSI,GEM33(J),NCHI,FWT,1)                          
         CALL SPLINE(NPSI,CS,FWT,0.0,0.0,3,H1,H2,H3,H4)                 
...
C                                                                       
     ...
     >                                       RR2GPDSR2,IR2GPDSR2)       
C-----------------------------------------------------------------------
C R**2*GRAD.PSI**2*GRAD.PSI*(GRAD.THETA)**2 *DS R**2 --> RR2GGG2DSR2,IR2
C-----------------------------------------------------------------------
      DO J=1,NCHI                                                       
         CALL SGCOPY(NPSI,GEM33(J),NCHI,FWT,1)                          
         CALL SPLINE(NPSI,CS,FWT,0.0,0.0,3,H1,H2,H3,H4)                 
...
C-----------------------------------------------------------------------
C   GRAD.PSI**2*(GRAD.PSI*GRAD.THETA)**2 *DS R**2 --> RGPGPT2DSR2,IGPGPT
C-----------------------------------------------------------------------
      DO J=1,NCHI                                                       
         CALL SGCOPY(NPSI,GEM33(J),NCHI,FWT,1)                          
         CALL SPLINE(NPSI,CS,FWT,0.0,0.0,3,H1,H2,H3,H4)                 
...
C                                                                       
     ...
     >                                           RDSR2OR2,IDSR2OR2)     
C-----------------------------------------------------------------------
C  R**2*GRAD.PSI*GRAD.THETA*GRAD.PSI**2 *DS R**2 -->RR2GPTGPDSR2,IR2GPTG
C-----------------------------------------------------------------------
      DO J=1,NCHI                                                       
         CALL SGCOPY(NPSI,GEM33(J),NCHI,FWT,1)                          
         CALL SPLINE(NPSI,CS,FWT,0.0,0.0,3,H1,H2,H3,H4)                 
...
C-----------------------------------------------------------------------
C R**2 * GRAD.PSI*GRAD.THETA**2 * DS R**2  -->  RR2GPGT2DSR2,IR2GPGT2DSR
C-----------------------------------------------------------------------
      DO J=1,NCHI                                                       
         CALL SGCOPY(NPSI,GEM33(J),NCHI,FWT,1)                          
         CALL SPLINE(NPSI,CS,FWT,0.0,0.0,3,H1,H2,H3,H4)                 
...
C-----------------------------------------------------------------------
     ...
     >                                           RDTR2OR4,IDTR2OR4)     
C-----------------------------------------------------------------------
C  R**2*(GRAD.PSI*GRAD.THETA)**2*D(R2)/D(THETA)--> RR2GPGT2DTR2,IR2GPGT2
C-----------------------------------------------------------------------
      DO I=1,NGES                                                       
...
C-----------------------------------------------------------------------
C  R**2*(GRAD.PSI*GRAD.THETA)**3*D(R2)/D(THETA)--> RR2GPGT3DTR2,IR2GPGT3
C-----------------------------------------------------------------------
      DO I=1,NGES                                                       
...
C-----------------------------------------------------------------------
C  R**2*(G.PSI*G.THETA)**2*GP**2*D(R2)/D(THETA)--> RR2GGG2DTR2,IR2GGG2DT
C-----------------------------------------------------------------------
      DO I=1,NGES                                                       
...
C-----------------------------------------------------------------------
C GRAD.PSI**2*(GRAD.PSI*GRAD.THETA)*D(R2)/D(THETA)--> RGPGPGTDTR2,IGPGPG
C-----------------------------------------------------------------------
      DO I=1,NGES                                                       
...
C     H(QUA) * FACT * H(CUB)   FUER DIE GLEICHUNGEN                     
C     ----------------------                                            
C     A(2,4),  A(2,7), A(3,4), A(3,5), A(2,5), A(6,5), A(6,7), A(6,4), A
C      v2A2,    v2p,    A1A2,   A1A3,   v2A3,   v3A3,   v3p,    v3A2,  A
C                                                                       
...
C     H'(CUB) * FACT * H(CUB)   FUER DIE GLEICHUNGEN                    
C     -----------------------                                           
C     A(1',4), A(5',4), A(1',7), A(7',1), A(5',5), A(1',5), A(4',4), A(4
C     dv1A2,    dA3A2,   dv1p,    p'v1,    dA3A3,   dv1A3,  dA2A2,    dA
C                                                                       
